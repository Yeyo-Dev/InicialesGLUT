...
-Serafín Velázquez Andrea Lizeth
...
void lineasDivisoras (void){
    //Lineas para dividir nuestros espacios de trabajo
    //Donde se pondran las iniciales de cada quién
...
void lineaH(int x, int y, int largo){
    //Función para hacer una linea horizontal ingresando sus coordenadas y el largo que tendrá
    glVertex2i (x,y);
    glVertex2i (x+largo, y);
}
void lineaV(int x, int y, int largo){
    //Función para hacer una linea vertical ingresando sus coordenadas y el largo que tendrá
    glVertex2i (x,y);
    glVertex2i (x,y+ largo);
}
void lineaDiagonalID(int x, int y, int largo){ 
    // Diagonal asi /
    //De 45° sólo usando coordendadas y el largo
    glVertex2i (x,y);
    glVertex2i (x+largo,y+ largo);
}
void lineaDiagonalDI(int x, int y, int largo){ 
    /* Diagonal asi \ */
    //De 45° sólo usando coordendadas y el largo
    glVertex2i (x,y+largo);
    glVertex2i (x+largo,y);
...
void LetrasIniciales(void){
    //Definimos las posiciones de las letras para que su acomodado sea más sencillo
    int fila[] = {120,70,15};
    ...
    glBegin(GL_LINES);
        lineasDivisoras();//Ponemos las linas para dividir el espacio
        //Las letras son de 16 x 16
        //Establece las coordenadas entre ellas con diferencia de 17
        //Diaz Cruz Esteban David
        glColor3f (0.0, 0.0, 0.0 ); // Fija color negro
        ...
        //Echeverria Valencia Greta Alitzel
        glColor3f (1.0, 0.2, 0.6 ); // Fija color rosa
        ...
        //Torres Gallardo Angel Gabriel
        glColor3f (0.0, 0.8, 0.8 ); // Fija color azul verdoso
        ...
        //Vargas Hernandez Luis Abraham
        glColor3f (0.8, 0.2, 0.4 ); // Fija color vino
